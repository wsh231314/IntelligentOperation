...
public class SikuliIDE extends JFrame implements InvocationHandler {
    ...
    private static MainApplet applet = null;
    
    private static Container parentWindow = null;
    
    private static boolean flgCloseable = false;
    
    private static String strScriptStaticPath = "";
    ...
    public static void showIDE() {
        ...
        if (!"".equals(strScriptStaticPath)) {
        	loadScripts = new String[] {strScriptStaticPath};
        	sikulixIDE.restoreSession(0);
        	strScriptStaticPath = "";
        	loadScripts = null;
        }  else {
        	sikulixIDE.doNewTabPane();
        }
    }
     ...
     * @param strPath
     */
    public static void setParentWindow(Container parent, String strPath, String strScriptDescription, String strScriptId) {
        parentWindow = parent;
        strScriptStaticPath = strPath;
        sikulixIDE.getNameTextField().setText(strScriptDescription);
        nameDescription = strScriptDescription;
    ...
    public static void hideIDE() {
        ...
        if (parentWindow != null) {
        	try {
				Method refreshMethod = parentWindow.getClass().getDeclaredMethod("searchDetailList", null);
				refreshMethod.invoke(parentWindow, null);
    ...
    public static void runAlone(String[] args) {
...
//        }
        ...
        sikulixIDE.initSikuliIDEAndNotShow(args);
        sikulixIDE.showMain();
    }
    ...
    private boolean saveSession(int action, boolean quitting) {
        ...
        for (int tabIndex = 0; tabIndex < nTab; tabIndex++) {
            try {
                ...
                } else if (codePane.isDirty() || !nameText.getText().trim().equals(nameDescription)) {
...
//      codePane.getSrcBundle();
        ...
        public boolean doSaveIntern(int tabIndex) {
            ...
            String fname = null;
            try {
                fname = codePane.saveFileWithDescription(nameText.getText());
                if (fname != null) {
                    setFileTabTitle(fname, tabIndex);
                } else {
                    retval = false;
    ...
    protected boolean checkDirtyPanes() {
        for (int i = 0; i < tabPane.getTabCount(); i++) {
            try {
                EditorPane codePane = getPaneAtIndex(i);
                if (codePane.isDirty() || !nameDescription.equals(nameText.getText().trim())) {
...
//    }
   ...
   */
    ...
    private void initSikuliIDEAndNotShow(String[] args) {
        sikulixIDE.ideSplash.showStep("Reading Preferences");
        prefs = PreferencesUser.getInstance();
        //prefs.exportPrefs(new File(runTime.fUserDir, "SikulixIDEprefs.txt").getAbsolutePath());
        if (prefs.getUserType() < 0) {
...
// RaiMan not used		initSidePane(); // IDE UnitTest
        ...
        nameLabel.setBounds(new java.awt.Rectangle(0,10,100,20));
        nameLabel.setText("Script Name:");
        
        // TextField 
        nameText = new JTextField();
        nameText.setBounds(new java.awt.Rectangle(140,10,120,20));
        namePanel.add(nameLabel, BorderLayout.WEST);
        namePanel.add(nameText, BorderLayout.CENTER);
        codeAndUnitPane.add(namePanel, BorderLayout.NORTH);
// RaiMan not used		codeAndUnitPane.add(_sidePane, BorderLayout.EAST);
        if (prefs.getPrefMoreMessage() == PreferencesUser.VERTICAL) {
            _mainSplitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, codeAndUnitPane, msgPane);
        } else {
