...
public class ComDao extends BaseDao {
	...
	private static final String MAIL_SEQ_FILE = "MAIL_SEQ.xml";
	
	private static final String JOB_SEQ_FILE = "JOB_SEQ.xml";
	...
	public String getMailSeq() {
		...
		try {
			SAXReader reader = new SAXReader();
			String seqFilePath = seqDir.concat(MAIL_SEQ_FILE);
			File file = new File(seqFilePath);
			if (!file.exists()) {
				util.createBlankXml(seqFilePath, CommonConstant.PATTERN_CUSTOMER);
			}
			fis = new FileInputStream(file);
			Document document = reader.read(fis);
			String strXPATH = "//".concat(CommonConstant.PATTERN_CUSTOMER);
			util.preCheckAddDoc(document, "", strXPATH);
			Node node = document.selectSingleNode(strXPATH);
			//
			String seq = node.getText();
	...
	public String getJobSeq() {
		...
		try {
			SAXReader reader = new SAXReader();
			String seqFilePath = seqDir.concat(JOB_SEQ_FILE);
			File file = new File(seqFilePath);
			if (!file.exists()) {
				util.createBlankXml(seqFilePath, CommonConstant.PATTERN_CUSTOMER);
			}
